...
var WorkerInfo = class extends AsynchronouslyCreatedResource {
  constructor(worker, port, workerId, onMessage) {
    super();
...
var ThreadPool = class {
  constructor(publicInterface, options) {
    ...
    this.workerIds = new Map(new Array(options.maxThreads).fill(0).map((_, i) => [i + 1, true]));
  ...
  _addNewWorker() {
    ...
    const isWorkerIdsCompatible = [...workerIds.values()].filter((isIdAvailable) => isIdAvailable === false).length === this.workers.size;
    const __dirname2 = dirname(fileURLToPath(import.meta.url));
    let workerId;
    workerIds.forEach((isIdAvailable, _workerId2) => {
      if (!isWorkerIdsCompatible) {
        const worker2 = workers.find((worker3) => worker3.workerId === _workerId2);
        if (!worker2) {
          workerIds.set(_workerId2, true);
          isIdAvailable = true;
        }
      }
