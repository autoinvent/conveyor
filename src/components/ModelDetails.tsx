...
function ModelDetails({
  ...
  return (
    <Container>
      <Form
        onSubmit={handleSubmit(mode === CREATE_MODE ? createData : updateData)}
      >
        <ListGroup>
          {detailFields.map(
            ({ fieldName, fieldValue, fieldType, fieldLink }) => {
              if (fieldName === 'id') {
                return (
                  <ListGroup.Item
                    key={`model-detail-${fieldName}-list-group-item`}
                  >
                    <Row>
                      <Col>id:</Col>
