...
import { CardContent, CardHeader, CardTitle, CardDescription, CircleCard, Deck, CardField } from '../Deck';
...
export const Home = () => {
  ...
  const { data, error, isLoading, isSuccess, isError, operationName } = useSearchQuery({ searchValue })
  const [searchedModels, setSearchedModels] = useState<Record<string, SearchResult[]>>({})
      ...
      <Deck>
        {
          Object.keys(searchedModels).map((modelName) => {
            return (
              <CircleCard key={modelName} onDoubleClick={() => navigate({ to: `/${modelName}` })}>
                <CardHeader>
