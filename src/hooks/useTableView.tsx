...
export type SortSequence = SortDirection[];
...
export type UseTableViewReturnType = ReturnType<typeof useTableView>;

export const useTableView = ({
  initialTableView = {},
  sortSequence = [SortDirection.ASC, SortDirection.DESC, SortDirection.NONE],
}: UseTableViewOptions = {}) => {
  ...
  */
  const getCurrentSort = useCallback(
    (field: string): SortDirection => {
      const sort = useStore(tableViewStore, (state) => state.sort);
  ...
  const nextSort = useCallback(
    (field: string) => {
      ...
      const nextSortHelper = (
        sortArray: string[],
        currIndex: number,
        currSortKey: SortDirection,
      ) => {
        const nextSortKey =
          sortSequence[
            (sortSequenceMap[currSortKey] + 1) % sortSequence.length
          ];
  ...
  */
  const addFilter = useCallback(
    (filter: TableViewFilter, filterGroup: number = 0) => {
      tableViewStore.setState((state) => {
        ...
        if (filterGroup > newFilter.length || filterGroup < 0) {
          if (newFilter.length === 0) {
            ...
            throw new Error(
              `filterGroup must be between 0 and ${newFilter.length}!`,
            );
          }
        }
  ...
  const swapFilter = useCallback(
    (
      ...
      tableViewStore.setState((state) => {
        const newFilter = state.filter ? [...state.filter] : [];
        if (
          filterGroup1 >= newFilter.length ||
          filterGroup1 < 0 ||
          filterGroupIndex1 >= newFilter[filterGroup1].length ||
          filterGroupIndex1 < 0 ||
          filterGroup2 >= newFilter.length ||
          filterGroup2 < 0 ||
          filterGroupIndex2 >= newFilter[filterGroup2].length ||
          filterGroupIndex2 < 0
        ) {
        ...
        newFilter[filterGroup1][filterGroupIndex1] =
          newFilter[filterGroup2][filterGroupIndex2];
