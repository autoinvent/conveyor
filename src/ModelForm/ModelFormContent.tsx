...
export const ModelFormContent = Object.assign(({ prefilled, children, className, ...props }: ModelFormContentProps) => {
  const { selected: fields } = useModelForm((state) => state.fields)
  const fieldNames: string[] = fields.map((field: Field) => field.name)
  return (
    <div className={twMerge("flex space-x-6", className)} {...props}>
