...
export const useRemoveAlert = () => {
    ...
    if (alertsStore === undefined) throw new Error('useRemoveAlert must be used within AlertStoreContext.Provider')

    return (alertId: string) => {
        alertsStore.setState((state) => {
            const newAlerts = state.alerts.filter((alert) => alert.alertId !== alertId)
            return {
                ...state,
                alerts: newAlerts
            }
