  ...
  ({ data, columnIds, children, className, ...props }: TableProps) => {
    ...
    return (
      <TableStoreContext.Provider value={tableStore}>
        <table
          className={twMerge('bg-[--fg-color] table-auto rounded w-full border-separate border-spacing-0 relative overflow-x-auto mb-2 overflow-hidden transition-colors outline outline-2 outline-[--border-color] border-[--border-color]', className)}
          {...props}
        >
          {children === undefined ? (
            <>
